...
### 2026-09-05 — v5.9 신설: Rule 4 를 구현 작업 밖으로 열고 분석 형판 추가 (배포)

61줄의 스코프 단어를 바꾸고 형판을 하나 더했다. **이 회차의 발단은 다른 프로젝트 세션이 보내온 지침 개선 제안 4건인데, 그 4건은 전부 기각됐고 채택된 것은 감사 과정에서 나온 별건이다.**

```
61: … before implementation:  →  … before starting:
66: + - "Analyze X" → name the question it must answer and the evidence
                      that would settle it
```

**기각 4건** — ① Rule 1 에 "환경 상태 질문은 설명 전에 그 상태를 먼저 읽는다" 신설 → **G2 ❌**, 96·103·127줄이 문자 그대로 덮는다("7일 롤링 창"은 96줄 열거의 version fact 다). ② 확실 주장에 근거를 문장 안에 표기 → G2 는 ✅(219줄이 추적의 존재만 요구하고 노출은 요구하지 않는다)이나 **G3 ❌** — 세션 밖 일반 사실에 댈 출처가 없어 그것들을 전부 "추정 — 미검증"으로 달면 라벨이 거짓이 된다. ③ 두 번 반증 시 세 번째 가설 금지 → **G2 ❌**, 194줄이 첫 반증에서 이미 잡는다. 194줄이 요구하는 것은 두 번째가 아니라 첫 번째다. ④ Gate 2 자동 해결 → ②에 종속, 독립 내용 없음. 사용자가 "환경설정 때문에 지침을 추가하고 싶지 않다"고 해 ② 를 좁힌 대체안(219줄에 환경 상태 예외)도 철회했다.

**채택안은 작업 유형 커버리지 렌즈에서 나왔다.** 불릿·`The test`·`Anti-pattern` 103개를 세어 작업별로 분류한 결과 42개가 작업 특정이고, 그 분포가 **코딩 26 · 테스트 9 · 디버깅 6 · 분석 1 · 문서 0** 이다(나머지 61개는 전 작업 공통). 어휘 실측이 같은 것을 확인해 준다 — 파일 전체에 `document` 0회, `analysis`·`analyze` 0회, `investigate` 1회(디버깅), `review` 3회(전부 diff·코드 리뷰·개수 주장).

분석·문서를 막는 것은 스코프 단어 셋이다 — 61줄 `before implementation`, 140줄 `Before adding or modifying code`, 41줄 `When editing existing code`. **셋 중 61줄만 고친 이유는 나머지 둘의 불릿이 전부 코드 어휘(exports·callers·imports·dead code)라, 스코프만 넓히면 문서 작업에서 실행할 수 없는 지시가 되기 때문이다.**

**형판을 함께 넣지 않으면 실효가 0에 가깝다는 것이 이 회차의 발견이다.** 첫 권고는 61줄 한 단어였는데, 사용자의 "단어 하나만 교체하면 되나"라는 물음에 63~65줄을 다시 보고 뒤집었다 — 형판 셋이 전부 코드 변경이라 스코프를 넓혀도 분석 작업이 맞출 칸이 없다. **140줄·41줄을 기각할 때 쓴 근거를 61줄에는 적용하지 않은 것이 첫 권고의 결함이다.**

**이득의 크기를 낮춰 적는다** — Rule 4 의 나머지 7개 단위(절단·계획·`A plan is not a deliverable`·`The test`)는 스코프 단어가 없어 분석 작업에서 이미 발동 중이었다. 61줄이 막고 있던 것은 **성공 기준 정의 한 단계**뿐이다. 134줄도 `Before delivering an implementation or a diagnosis` 로 분석 산출물을 이미 명시하고 있어, 비어 있던 것은 착수 시점이지 분석 전체가 아니다.

**대가 1건** — 짧은 분석 질문("이 함수 뭐 하는 거야?")에도 성공 기준을 세우려 들 수 있다. 완충은 222줄 `a simple question gets one to three sentences` 하나뿐이고, 181줄 `Mechanical, easily-reversible changes get a compile/lint pass` 는 코드 한정이라 여기서는 받쳐 주지 않는다. **관찰 대상이다.**

**미포함 2건.** ① **문서 형판** — 대칭으로는 넣어야 맞지만 분석 쪽만 사례가 쌓여 있고 문서 작업의 완료 기준 실패는 확인하지 못했다. 08-29 항목대로 한 회차에 변수 하나만 움직여야 귀인이 된다. ② **140줄·41줄** — 위 이유.

**형판의 실효는 추정이며 미검증이다.** 설계상으로는 오늘 실패(주간 한도 설명)를 잡는다 — 착수 시점에 "답해야 할 질문 / 그것을 가릴 증거: `/usage` 화면"이 나왔다면 화면을 먼저 봤을 것이다. 다만 이 유추는 08-29 항목이 "설계상 유추 외에 댈 것이 없었다"며 되돌린 것과 같은 형태다.

`check.sh` 3/6/14 로 v5.8 과 출력 문자 단위 동일(신규 위반 0건). 파일 3593 → 3610단어(+0.47%), 246 → 247줄. 파급 없음 — `implementation` 의 나머지 출현 4곳(109·134·156·157)은 대상이 달라 끊어질 참조가 없고, Rule 4 를 참조하는 곳은 Gate 1 하나인데 계획 단계만 가리킨다. `before starting` 은 178줄이 이미 쓰는 어휘라 신규 용어가 아니다. **이번에는 제자리 수정이 아니라 새 버전 파일로 분리했다**(사용자 지시) — `ClaudeCode/CLAUDE.v.5.9.md`, `~/.claude/CLAUDE.md` 배포 완료, sha1 `584dc894…` 바이트 일치 확인.

**부수 확인 1건** — `doc-writer` 실제 배포처는 7곳이고 08-14 항목의 5곳 기록은 낡았다 — `Work/` 안 6곳(`rules` 루트·`axplatform`·`Incubation`·`agent-composer-core`·`agent-studio`·`bmad2`)에 `Work/` 밖 `Downloads/kt_academy` 하나다. `Work/` 에서 `.claude/` 를 가진 프로젝트가 16곳이므로 10곳에서는 문서 작업을 관할하는 것이 글로벌·스킬 양쪽에 없다. **문서 공백은 지침이 아니라 배포로 메울 자리다.**

### 2026-08-30 — v5.8 217줄 앵커의 동사 교체 (제자리 수정, 배포)

Response Discipline 첫 문장의 독자 앵커를 `understands it top-to-bottom` 에서 `can decide what happens next` 로 바꿨다. **명사(`senior colleague`)는 그대로 두고 동사만 바꾼 것이 이 회차의 결론이다.**

```
as-is: Respond so an intelligent senior colleague understands it
       top-to-bottom on the first pass.
to-be: Respond so an intelligent senior colleague can decide what
       happens next on the first pass.
```

발단은 사용자의 "`senior colleague` 를 `team lead` 로 바꾸면 어떨까"였고, 의도는 **실무자가 팀장에게 보고할 때의 선별 압력**(다음 할 일 판단에 필요한 것만)이었다. 감사에서 처음 낸 안은 `an intelligent, technical team lead` 였다 — 명사를 바꾸되 `senior` 가 지던 깊이 가정을 `technical` 로 받는 형태. **그 안은 사용자의 역제안으로 뒤집혔고, 뒤집힌 것이 옳다.**

**핵심 발견은 바꿔야 할 곳이 명사가 아니라 동사였다는 것이다.** `top-to-bottom` 은 "빠짐없이 이해시켜라"라서, 명사만 바꾸면 완결 이해 요구가 그대로 남는다. 보고의 본질은 팀장이 전부 이해하는 것이 아니라 다음을 정할 수 있는 것이고, 그 차이를 만드는 단어가 `top-to-bottom` 이었다. 첫 감사가 이 조항을 깊이 가정으로만 읽어 명사 쪽을 본 것이 오독이다.

**명사를 안 바꿔서 지킨 것 셋** — ① `senior` 의 깊이 가정 유지(이 신호는 파일에서 217줄에만 있다. 182·222·224·227 이 이미 네 번 담고 있는 "결정자" 신호와 달리 대체 자리가 없다) ② 동료→상급자 프레이밍이 push back 에 완충을 붙일 위험(미관측, Rule 1·6 이 상쇄) 회피 ③ **변수 하나만 움직여 귀인이 가능해진다.** ③ 은 08-29 항목이 게이트 안을 되돌리며 남긴 사유 1번과 같은 원칙이고, 이번에는 사용자가 적용했다.

**대가 1건** — `senior colleague` 는 대등한 동료라 "결정하는 사람"과 반 칸 어긋난다. 다만 신설된 긴장이 아니다: 182줄 `filtering is the reader's call`·222줄 `when the reader has to choose`·227줄 `the sentence the reader acts on` 이 이미 독자를 결정자로 다루고 있었고 **217줄만 안 그랬다.** 이번 수정으로 정의와 사용처가 맞는다.

**완결성 하한 제거가 이 회차에서 가장 위험한 자리다.** `top-to-bottom` 이 지던 몫을 206줄 Rule 12(`Silence is not a valid state for a part`)와 Final Gate 1 `Complete` 가 명시로 붙들고 있음을 파일에서 확인했다. 217줄이 유일한 방벽이 아니었다. 남는 위험은 "결정에 필요 없다"는 판정이 자기 재량이라는 것 — 08-20 A건의 `simple question` 판별과 같은 취약점을 공유한다. **관찰 대상이다.**

**미포함 3건.** ① **182줄(Rule 10) 순서 강제** — `anything you noticed gets surfaced with a severity label` 뒤에 `ordered by what the reader must decide` 를 넣는 안. 문안·검증 모두 준비됐으나(check.sh 동일, 27→34단어) **217줄과 같은 방향으로 밀어 귀인이 불가능해지므로 다음 회차로 미뤘다.** 이 안이 드러낸 것은 사라지지 않는다 — 182줄 `filtering is the reader's call, not yours` 와 224줄 `cut any detail that wouldn't change what the reader does next` 가 **같은 대상에 반대 방향**이고, 화해시키는 문장이 파일에 없다. `filtering is the reader's call` 자체를 완화하는 안은 은폐 방지(Silent Shrink·Optimistic Path)를 파는 거래라 권하지 않았다. ② **명사 `team lead` 교체** — 위 이유. ③ **`# Language` 프롬프트 문안 편입** — 아래 별도 항목.

**렌즈 전수 점검 결과**: 보고 내용을 강제하는 요소 20개(Response Discipline 9 · Final Gate 7 · 규칙 조항 4 — 134·182·195·206)를 "독자의 다음 결정을 바꾸는가 vs 작성자의 자기증명인가"로 훑어 **2건 적발, 18건 유지**. Final Gate 4·5 는 자기증명 형태지만 `items 4 and 5 are the only risk-scaled ones` 로 고위험 턴에만 발동하고, 그 턴이야말로 팀장이 "뭘 확인했나"를 실제로 묻는 자리라 유지했다. 195줄도 `low-stakes answers pass silently` 가 이미 붙어 있어 같은 이유로 유지.

`check.sh` 3/6/14 로 수정 전과 출력 문자 단위 동일(신규 위반 0건). 217줄 31 → 33단어(+6.5%), 파일 3591 → 3593(+0.06%), 246줄 유지. `top-to-bottom` 은 파일 내 유일 등장이라 끊어질 참조가 없다. 버전은 v5.9 로 올리지 않고 v5.8 을 제자리 수정했다(사용자 판단, 08-29 선례). `~/.claude/CLAUDE.md` 배포 완료, sha1 `65171f8f…` 바이트 일치 확인.

### 2026-08-30 — `# Language` config 프롬프트 조사와 편입 기각 (3건 전부)

Claude Code v2.1.251 바이너리에서 `/config` → Language 설정이 주입하는 프롬프트를 추출했다. 템플릿 함수 `Sue(e)`, 주입 지점 `yc("language", …)` — `env_info` 뒤 output style 앞이며, 정적 시스템 프롬프트 모드에서는 `language` 타입 첨부로 같은 문안이 전달된다. 값은 `ve()` 가 정규화한다: `korean`→Title Case, `ko`→`Intl.DisplayNames` 로 영어 이름, `kr`→별칭표(`kr→ko`·`jp→ja`·`cn→zh`·`tw→zh-Hant`) 경유. **프롬프트에 들어가는 문자열은 `한국어` 가 아니라 영어 단어 `Korean` 이다.**

문안 세 문장을 225줄 편입 후보로 감사해 **셋 다 보류·기각**했다.

| 문장 | 판정 | 사유 |
| --- | --- | --- |
| S1 `Always respond in Korean` | 보류 | G1 ❌ — 언어 이탈 사건 기록 0건(`.remember` 75파일·`CLAUDE.local.md` 2회 검색) |
| S2 `Technical terms and code identifiers … original form` | 보류 | 이 저장소 `CLAUDE.md` §평이 설명 모드(`전문용어 치환`)와 **정면 충돌**. 좁힌 판(식별자·규칙명·경로 한정)은 충돌 없으나 값도 미확인 |
| S3 발음 부호·정서법 | 기각 | 한국어에 발음 부호가 없어 **발동할 상황 자체가 없다**(G3 공허 충족) |

**S1 에 대한 실측 반대 증거가 같은 세션에서 나왔다** — config 가 default 인 상태에서 사용자가 영어로 `hello` 라고 인사했는데 답변이 한국어로 나왔다. 조항이 하려던 일을 이미 하고 있다는 뜻이다. 다만 **가장 약한 경우**다: 한국어 세션 중 영어 한 단어라 언어 전환 신호로 보기 어렵고, 225줄 라벨 `**Korean**:` 이 이미 한국어 출력을 전제해 조항 없이도 그쪽으로 밀린다. **판별이 되는 시험은 새 세션의 첫 입력을 영어로 쓰는 것이다 — 미실시.**

**비용 비교 1건은 기록 가치가 있다.** 강제가 정말 필요해지면 config 보다 지침이 싸다 — config 는 매 턴 62단어를 주입하고 그중 약 30단어(라틴 발음 부호 예시)가 한국어에서 죽은 문장인 반면, 같은 효과의 조항은 9단어다. **"하네스 기능을 지침으로 중복하지 말라"는 평소 원칙이 이 건에서는 뒤집힌다.**

첫 감사에서 낸 S1 무결성 지적(`**Korean**:` 라벨 안에 "한국어로 답하라"를 넣으면 조건절이 자기참조)은 **줄 전체를 재정의하는 형태에서는 성립하지 않는다.** 재정의 시 217줄이 Response Discipline 을 `user-facing responses` 로 한정하므로 PR 본문·커밋 메시지가 사정권에서 빠지는 이점도 확인했다 — config 문안의 `all … communications` 에는 그 한정이 없다.

### 2026-08-29 — v5.8 에 제목 자리 추가 후 배포 (한국어 비유 억제, 제자리 수정)

미배포 상태이던 v5.8 의 225줄 열거에 제목 자리를 더해 배포했다. 발단은 사용자의 "자꾸 비유법을 쓰는데 좀 더 강화할 수 없나"이고, **직전 세션의 답변에서 실제 위반 2건이 나온 것이 근거다** — 섹션 제목 `붙잡아 둘 것 둘`(공간·부착 갈래)과 `실측 전부가 한 형상 뒤로 밀렸습니다`(물리 갈래). 08-28 항목이 G1 을 ❌ 로 두며 "한국어 대화 답변의 사례를 기록에서 못 찾았다"고 적었던 그 공백이 하루 만에 채워졌다.

```
225: … out of plain slots — state, verdict, result.
   → … out of plain slots — state, verdict, result,
       and the headings that label them.
```

**Final Gate 6 에 한국어 어휘 절을 신설하는 안을 함께 배포했다가 같은 세션에서 되돌렸다.** 문안은 `In Korean, no figurative word sits where a plain predicate would do.` 였고, 감사 판정은 적용이었다. 사용자의 "게이트까지 두는 건 과하지 않나"라는 지적으로 재검토해 삭제했으며, **되돌림의 근거 셋이 이 회차의 기록 가치다.**

1. **두 변경을 한 회차에 넣으면 귀인이 불가능하다.** 225줄 열거 자체가 어제 만들어져 아직 시험되지 않았다. 그 값을 모르는 상태에서 더 비싼 조항을 같이 넣으면, 앞으로 위반이 줄어도 어느 쪽 덕인지 말할 수 없다.
2. **반대 증거가 있다.** 어제 실패한 턴도 게이트 6 을 통과시켰다. 이미 통과시킨 항목에 문구를 더 넣는 것이 왜 이번에는 통할지, 설계상 유추 외에 댈 것이 없었다. 감사 자체가 이 효과를 "미확인"으로 표시했는데도 적용으로 판정한 것이 판단 착오다.
3. **검사 단위가 한 칸 내려간다.** 게이트 6 의 나머지 절은 전부 문단 배치·길이·형태를 본다. 거기에 한 언어의 단어 선택이 들어가면 저위험 턴까지 포함해 매 턴 그 비용을 낸다. §Design Decisions 의 구판 v5.4 항목이 경고한 "특정 사건을 글로벌 패턴으로 승격하려는 충동"과 같은 모양이다.

**게이트 안은 폐기가 아니라 보류다** — 225줄 열거만으로 부족하다는 관찰이 나오면 그때 근거를 갖고 다시 올린다.

**이 회차의 진단 자체는 유효하다.** 08-28 이 고치려던 자리가 판정(어느 자리가 평범한 자리인가)인데, 실제로 실패한 단계는 인지였다 — 사용자가 묻자 모델이 즉시 "비유라서 뜻이 바로 안 읽힙니다"라고 답했으므로 지식은 있었고 쓰는 순간에 조항이 발동하지 않았다. 제목 자리 추가는 인지를 돕지 않고 판정 범위만 넓히므로, **어제 사건의 재발을 막는다는 보장은 없다. 관찰 대상이다.**

**기각 1건** — doc-writer §7.3 의 네 갈래(신호·색 / 물리·재난 / 의인화 / 공간·부착)를 225줄에 이식해 인지를 돕는 안. **G3(충족) 탈락** — "물리 동작에서 빌려온 단어"로 잡으면 걸리다·넣다·빼다·잡다 같은 사문화된 비유가 전부 걸린다. 한국어 기술 산문의 기본 어휘라 충족 불가에 가까워지고, 조항 전체의 구속력을 깎는다. doc-writer 는 40행 대응표와 `check.sh` 가 경계를 대신 그어 주지만 글로벌에는 그 장치가 없다.

**충돌 1건은 게이트 안과 함께 사라졌다.** 이 저장소 `CLAUDE.md` 의 평이 설명 모드는 "비유는 하나만"으로 비유를 명시 요구하는데, 삭제한 문안은 조건절 `where a plain predicate would do` 로 이것을 지나가게 했다. 조건절 없이 금지형으로 썼다면 정면 충돌이었다 — **다음에 같은 안을 다시 올린다면 조건절이 필수다.**

`check.sh` 3/6/14 로 v5.7 과 출력 문자 단위 동일(신규 위반 0건). 225줄 25 → 35단어(+40%), 파일 3581 → 3591(+0.28%), 246줄 유지. 버전은 v5.9 로 올리지 않고 v5.8 을 제자리 수정했다(사용자 판단). `~/.claude/CLAUDE.md` 배포 완료, sha1 `76e7660c…` 바이트 일치 확인. **배포본은 이제 v5.8 이다** — 08-28 항목의 "배포본은 v5.7" 서술은 이 회차로 무효다.

### 2026-08-28 — v5.8 신설: 225줄 `plain slots` 에 금지 자리 열거 (당시 미배포 — **08-29 회차에서 제목 자리를 추가해 배포됐다**)

`Keep figurative words out of plain slots` 의 `plain slots` 가 정의되지 않아 자기 재량 판정이던 것을 열거 셋으로 확정했다. 발단은 사용자가 이 조항과 doc-writer 의 비유 규정을 대조해 달라고 한 것이고, 대조 결과 doc-writer 는 판별 규격(§7.3 회피·권장 대응표 약 40행 + 비유 4갈래 표 + `check.sh` 0건 강제)을 갖는데 225줄은 원칙 한 문장뿐이라는 비대칭이 드러났다.

```
as-is: Keep figurative words out of plain slots.
to-be: Keep figurative words out of plain slots — state, verdict, result.
```

**감사에서 처음 낸 안(doc-writer §2.1·§7.3 정의를 그대로 이식)은 보류로 뒤집혔다.** 이식안은 `a figure belongs only where a concept is first introduced, and the next sentence returns to the exact term` 이었는데, 그 뒤 절이 비유를 쓸 때마다 문장을 하나 더 요구하는 데 반해 **바로 다음 문장이 `None of this adds length` 라 같은 줄 안에서 불변조건을 위반**한다. 52단어로 as-is 25단어의 두 배가 되는 것도 매 턴 상주 비용에서 회수되지 않는다.

**방향 자체가 틀렸다는 것이 이 회차의 발견이다.** 정의가 없던 것은 허용 자리가 아니라 금지 자리였다. 허용 쪽은 224줄 `keep the one example or contrast that makes an abstract point land` 가 이미 관할하므로, 허용 자리를 신설하면 224줄과 회색지대만 생긴다(`only` 가 224줄보다 좁아 추상 논점을 착지시키는 대조가 비유 형태일 때 반대 판정이 난다). as-is 는 `plain slots` 가 미정의라 이 긴장이 표면화되지 않았고, 이식안이 그것을 만들어 냈다.

**게이트는 허용 자리 신설분에만 걸었고 탈락했다** — G1 재현 ❌(이 세션에 확인한 비유 사건 2건이 모두 225줄 범위 밖이다. 08-22 `load-bearing` 은 영어 지침 본문이라 `Korean` 조항이 발동하지 않고, 08-14 doc-writer 어휘 19곳은 문서 산출물이라 216줄 `user-facing responses` 밖이다. 한국어 대화 답변의 사례는 기록에서 못 찾았다), G2 ⚠️ 판정 불가(대조할 실패 사례가 없다), G4 값 ❌(증분의 실효가 억제가 아니라 허용이다). 채택한 열거안은 기존 용어의 정의 신설이라 게이트를 건너뛴다.

**대가 1건** — 대화 답변의 문장 대부분이 상태·판정·결과 셋 중 하나라, 열거가 사실상 "한국어 답변에 비유를 쓰지 말라"에 가까워진다. 224줄의 예시·대조 허용이 유일한 반대 추이고 그 경계는 여전히 재량이다. **관찰 대상**이다.

**대조에서 함께 확인한 것 둘.** ① doc-writer 의 "쉬운 문장"은 평이화가 아니다 — SKILL.md:27 이 "이해를 돕는 방법은 평이화가 아니라 정밀화"로 못박고 일반 독자용 비유·기초 용어 해설을 산출물에서 배제한다. CLAUDE.md 쪽(220줄 `plain language`·224줄 `unexplained jargon` 금지)이 평이화에 반 칸 더 가깝다. ② 길이 방향이 반대다 — 225줄은 "길어지면 틀렸다"이고 writing-principles:152 는 "글자 수보다 이해를 우선해 한 박자 풀어 쓴다"이다. 224줄 `Stop cutting where a first-time reader would lose the thread` 가 완화하지만, 겹치는 자리는 대화창에 문서 초안을 인용할 때 하나다.

`check.sh` 3/6/14 로 v5.7 과 출력 문자 단위 동일(신규 위반 0건). 225줄 25 → 29단어(+16%), 파일 3581 → 3585(+0.11%), 246줄 유지.

**이번에는 제자리 수정이 아니라 새 버전 파일로 분리했다** — v5.7 을 원문으로 되돌리고(sha1 `af91ddfc…`, 08-22 항목이 기록한 값과 일치), 변경분은 `ClaudeCode/CLAUDE.v.5.8.md`(sha1 `f038d60b…`)에 담았다. **배포본 `~/.claude/CLAUDE.md` 는 v5.7 이다.** v5.8 은 위 "관찰 대상"을 더 확인한 뒤 배포한다 — 배포 전에는 v5.7 이 정본이므로, 다음 회차가 최신 버전 파일을 정본으로 삼는 관행을 이 항목보다 앞세우지 않는다.

### 2026-08-22 — v5.7 227줄 낙인어 교체 시도 후 원복 (왕복, 순변화 없음)

`The test` 마지막 절을 `Anything in neither group was never load-bearing.` → `Anything in neither group gets cut.` 으로 바꿔 배포했다가 **같은 세션에서 사용자 지시로 되돌렸다. 현행 배포본은 원문 그대로**(sha1 `af91ddfc…`, 일치 확인). 원복 시점에 사유는 기록되지 않았다 — 다음 회차가 같은 제안을 다시 올리면 이 항목을 먼저 읽는다.

아래는 시도 회차의 진단이며, 결론이 아니라 재검토용 자산으로 남긴다. **발단은 사용자가 이 문장을 읽고 "그래서 뭐 어쩌라는 건가"를 물은 것이다** — 지침 작성자 본인이 뜻을 못 세웠다.

원인은 낙인 어휘 하나였다. 13개 `The test` 의 마지막 절을 모아 보면 여섯은 동작을 직접 지시하고(`revert it`·`drop it`·`go run it`·`keep reading`·`sharpen them`·`label it or go check`), 나머지는 낙인으로 닫는다(`you've over-built`·`They protect nothing`·`= dropped`·`you haven't attacked`). **낙인형 중 `load-bearing` 만 건축에서 빌려 온 도메인 비유라, 뜻을 한 번 옮긴 뒤에야 처분에 도달했다.** 나머지 낙인어는 전부 일상 술어다. 227줄이 다른 12개와 성격이 다른 것이 아니라 어휘가 한 칸 멀었던 것이다.

**은유 중복 1건이 함께 해소됐다.** 앞 절이 이미 `the ones that only support it` 으로 "받친다"를 말하는데 `load-bearing` 이 같은 건축 은유를 한 번 더 썼다. `gets cut` 으로 바꾸면 은유는 `support` 하나만 남는다.

**기각 1건** — `cut anything in neither group` 처럼 명령형으로 가는 안. 앞 문장이 이미 `Name X, then Y` 명령형이라 세 번째까지 명령형이면 판정의 성격이 사라진다. 162줄 `They protect nothing`·211줄 `Unpointable = dropped` 가 비명령 판정형 선례라 `gets cut` 은 형식 일탈이 아니다.

**대가 1건** — 판정이 처분으로 확정되면서 맥락 문단의 삭제 압력이 올라간다. 224줄 `keep the one example or contrast that makes an abstract point land` 가 하한으로 남아 완화하지만, 예시 문장을 어느 무리에 넣을지의 긴장은 표면화된다(as-is 에도 있던 긴장이고 신설이 아니다).

감사 자체는 통과였다 — `check.sh` 3/6/14 로 as-is 와 출력 문자 단위 동일(신규 위반 0건), 227줄 23 → 22단어, 파일 3581 → 3580(−0.03%), 246줄 유지. **판정이 적용이어도 되돌아올 수 있다는 것이 이 회차의 기록 가치다.**
