...
| 구분 | 조항 |
| --- | --- |
| 채택 | 종결어미로 문장 끝내기 · 조사·어미로 관계 표시(명사 나열 금지) · 엠대시 → 콜론 · 비유적 어휘 회피 |
| 제외 | 문장 성분 생략 금지 · 부사·보조사·보조용언 "적극 활용" — 둘이 길이 증가의 원인 |
| 부분 | 한자어 조항은 "적극 활용"이 `report-style` §2.5(격식 한자어 회피)와 충돌해, 명사 나열 금지 효과만 가져옴 |

미끄럼 방지로 `None of this adds length` 문장을 붙였고, `prose` 한정어로 대화창에 인용하는 보고서 초안(`report-style` 의 명사형 종결 관할)과 경계를 뒀다. 감사 결과는 G1~G4 전부 통과, 축 이동은 D 정밀 쪽 작음뿐, `check.sh` 출력이 v5.3 과 동일해 불변조건 신규 위반 0건이다. 단어 3544 → 3595(+1.44%).

커밋 `8aa9301`, 배포 완료. 플러그인은 삭제하지 않고 `.claude/settings.local.json` 의 `enabledPlugins` 에서 비활성화했다 — 재검토 시 `true` 로 되돌리면 된다. **엠대시 조항만 문장 수를 늘릴 여지가 있어 1주 관찰 후 유지 여부를 재판정한다.**
