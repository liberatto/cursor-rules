...
- **v5.9 관찰 기간 — 2026-09-26 까지 동결** (2026-09-05): Rule 4 61줄과 `"Analyze X"` 형판을 그 전에 고치지 않는다. 이번에 변수를 하나만 움직였으므로 분석 답변이 달라지면 이 변경에 귀인할 수 있는데, 그 전에 문서 형판이나 140·41줄을 더하면 귀인이 불가능해진다(08-29·08-30 항목과 같은 원칙). **볼 것 둘** — ① 분석 요청에 착수 목표가 실제로 나오는가 ② 짧은 질문("이 함수 뭐 하는 거야?")이 무거워지는가. ①이 안 나오면 조항이 아니라 발동이 문제이고, ②가 나오면 222줄 완충이 약한 것이다

---

## Recent Changes

### 2026-09-05 — v5.9 신설: Rule 4 를 구현 작업 밖으로 열고 분석 형판 추가 (배포, 커밋 `b89f30b`)
...
**배경** — 지시문 103개를 작업 유형으로 분류했더니 작업 특정 42개의 분포가 코딩 26 · 테스트 9 · 디버깅 6 · **분석 1 · 문서 0** 이었다. 어휘 실측도 같다 — `document` 0회, `analysis`·`analyze` 0회. 61줄의 `before implementation` 이 성공 기준 정의를 구현 작업에만 묶어, 분석 작업이 착수 시점 규칙을 받지 못했다.

**한 단어로는 실효가 0이다** — 형판 셋이 전부 코드 변경이라 스코프만 넓히면 맞출 칸이 없다. 첫 권고(한 단어)는 140줄·41줄을 기각할 때 쓴 근거를 61줄에 적용하지 않은 결함이었고, 사용자의 "단어 하나만 교체하면 되나"로 뒤집혔다.

**이득의 크기를 낮춰 적는다** — Rule 4 의 나머지 7개 단위는 스코프 단어가 없어 분석 작업에 이미 걸리고 있었다. 새로 걸린 것은 **성공 기준 정의 한 단계**뿐이고, 134줄은 `or a diagnosis` 로 분석 산출물을 이미 명시하고 있었다.

**기각 4건**(외부 세션이 보내온 제안, 전부 환경 상태 관련) — ① 환경 상태 선확인 조항 → **G2 ❌** 96·103·127줄이 문자 그대로 덮는다 ② 확실 주장에 근거 인라인 표기 → **G3 ❌** 세션 밖 일반 사실에 댈 출처가 없어 라벨이 거짓이 된다 ③ 두 번 반증 시 가설 금지 → **G2 ❌** 194줄이 첫 반증에서 이미 잡는다 ④ Gate 2 자동 해결 → ②에 종속. **같은 제안이 다시 오면 이 항목을 먼저 읽는다.**

**미포함 2건** — 문서 형판(실패 사례 미확보), 140줄·41줄(불릿이 전부 코드 어휘라 스코프만 넓히면 실행 불가 지시가 된다).

**대가 1건** — 짧은 분석 질문에 성공 기준을 세우려 들 수 있다. 완충은 222줄 `a simple question gets one to three sentences` 하나뿐이고 그 판정도 재량이다.

`check.sh` 3/6/14 로 v5.8 과 출력 문자 단위 동일(신규 위반 0건). 3593 → 3610단어(+0.47%), 246 → 247줄. `~/.claude/CLAUDE.md` 배포 완료, sha1 `584dc894…` 바이트 일치. 부수 확인 — `doc-writer` 실제 배포처는 7곳으로 08-14 항목의 5곳 기록이 낡았다(`Work/` 안 6곳 + `Downloads/kt_academy`).

**형판의 실효는 추정이며 미검증이다** — 설계상 잡힌다는 유추뿐이고, 08-29 항목이 "설계상 유추 외에 댈 것이 없었다"며 되돌린 것과 같은 형태다. 그래서 **2~3주 관찰한다. 2026-09-26 까지 Rule 4 와 분석 형판을 건드리지 않는다** — 사유·관찰 항목은 §Design Decisions 참조.
...
- **문서 작업의 관할 공백**: `Work/` 에서 `.claude/` 를 가진 프로젝트 16곳 중 `doc-writer` 가 있는 곳은 6곳이다. 나머지 10곳에서는 글로벌 지침의 문서 전용 조항 0개에 스킬도 없다. **지침이 아니라 배포로 메울 자리다** — 어느 프로젝트에 심링크할지 판단 필요
- **v5.9 관찰 결과 판정 (2026-09-19~26)**: §Design Decisions 의 관찰 항목 둘을 확인해 (a) 유지 (b) 문서 형판 추가 (c) 되돌림 중 결정한다. 되돌림은 `cp ClaudeCode/CLAUDE.v.5.8.md ~/.claude/CLAUDE.md` 한 줄이며 v5.8 은 커밋 `c00bf36` 에 있다
