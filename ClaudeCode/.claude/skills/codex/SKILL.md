---
name: codex
description: Use when the user asks to run Codex CLI (codex exec, codex resume) or references OpenAI Codex for code analysis, refactoring, planning, validation, testing, or automated editing
---
...
1. Ask the user (via `AskUserQuestion`) which model to run (`gpt-5.2` or `gpt-5.2-codex`) AND which reasoning effort to use (`xhigh`, `high`, `medium`, or `low`) in a **single prompt with two questions**.
