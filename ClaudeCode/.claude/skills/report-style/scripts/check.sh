...
# 절 번호를 문장 안에서 지시어로 쓰는 형태 (§1.4) — `§4.2가 지적한`·`§1의 세 축`처럼
# 조사가 붙어 앞뒤 절을 되짚게 만드는 것. XREF 가 잡던 정형구 밖으로 새던 형태라 축을 나눠 둔다.
# ⚠ 외부 문서·법령의 절을 인용한 자리(`대상 문서 §4.3의 표`)는 오탐 — 출력 줄을 눈으로 확인한다.
XREF2='§[0-9][0-9.]*(이|가|은|는|을|를|의|에|에서|에는|이라|과|와)[ ,.·)]'
...
echo "[자립성 절 지시어] $(cnt "$XREF2")건"; hit "$XREF2" 8
[ "$(cnt "$XREF2")" != "0" ] && echo "  ↳ 절 번호 대신 가리키는 내용을 그 자리에 한 구절로 넣는다 (외부 문서 절 인용이면 오탐)"
echo "[#### 과깊이]      $(cnt '^####')건"
