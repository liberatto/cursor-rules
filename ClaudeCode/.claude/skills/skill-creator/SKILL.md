...
| 항목 | 설명 |
|------|------|
| 스킬 이름 | kebab-case (예: `code-review`, `api-generator`) |
| 스킬 설명 | 무엇을 하는가 + 언제 사용하는가 |
| 트리거 시나리오 | 어떤 요청에 트리거되어야 하는가 |

### 2단계: 실행 환경

| 항목 | 옵션 |
|------|------|
| 컨텍스트 격리 | `fork` (독립 서브에이전트) / 미지정 (현재 대화) |
| 에이전트 타입 | `Explore`, `Plan`, `general-purpose` / 미지정 |

### 3단계: 도구 권한

| 항목 | 설명 |
|------|------|
| 허용 도구 | Read, Write, Edit, Glob, Grep, Bash(패턴) 등 |
| 미지정 시 | 모든 도구 허용 |
...
| 항목 | 옵션 |
|------|------|
| 슬래시 메뉴 표시 | `true` (직접 호출 가능) / `false` (자동 감지만) |
