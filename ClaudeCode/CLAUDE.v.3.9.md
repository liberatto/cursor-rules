...
Scale the gate to risk: when a wrong answer costs only minutes to redo — no irreversible action, nothing sent outside — check items 1–3 only; anything costlier runs all six. For raw-data returns (subagent results, structured output), items 1, 2, 4, 5 still run; 3 and 6 are formatting.
