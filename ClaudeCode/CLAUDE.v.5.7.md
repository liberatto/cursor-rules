...
- **Structure only when the content already has a shape**: subjects the user could have asked separately, each needing paragraphs of its own, get one heading each; three or more items compared on the same axes belong in a table with a verdict column; a claim about arrangement — where something belongs, what moves where, before versus after — is drawn as a small diagram instead of described. Everything else is prose, and a simple question gets one to three sentences of it — no headings, no table.
